...
<div class="min-h-screen bg-white py-6 md:py-12 px-4 sm:px-6 lg:px-8 overflow-hidden">
    <div class="max-w-6xl mx-auto">
        ...
        {{-- Back Button (ANIMASI: MASUK DARI ATAS) --}}
        <div class="mb-6 md:mb-10 opacity-0 animate-fade-down">
            <a href="{{ route('catalog') }}" class="inline-flex items-center gap-2 text-slate-500 hover:text-blue-600 transition-colors font-semibold text-sm mb-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-start">
            ...
            {{-- Sisi Kiri: Gambar (ANIMASI: MASUK DARI KIRI & HOVER INTERAKTIF) --}}
            ...
            <div class="lg:col-span-7 space-y-6 md:space-y-8 text-center lg:text-left">
                ...
                {{-- Bagian Judul & Author (ANIMASI: MASUK DARI KANAN) --}}
                ...
                <div class="grid grid-cols-2 gap-3 md:gap-4 opacity-0 animate-fade-up-1">
                    ...
                    <div class="bg-slate-50 p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] border border-slate-100 hover:shadow-md transition-shadow duration-300">
                        ...
                        <p class="text-sm md:text-lg font-bold text-slate-800">{{ $book->isbn }}</p>
                    </div>
                </div>

                {{-- Sinopsis (ANIMASI: MASUK DARI BAWAH TINGKAT 2) --}}
                ...
                {{-- Button dengan Form Peminjaman (ANIMASI: MASUK DARI BAWAH TINGKAT 3) --}}
                <div class="pt-2 opacity-0 animate-fade-up-3">
                    ...
                    {{-- Tombol Action Utama - Real-time Action --}}
                    <div class="mt-6">
                        @if(Auth::check() && isset($isBorrowedByUser) && $isBorrowedByUser)
                            {{-- KONDISI 1: User sedang meminjam buku ini --}}
                        ...
                        @else
                            {{-- KONDISI 3: Buku lolos kriteria dan siap dipinjam --}}
                            @if(!Auth::check())
                                <a href="{{ route('login') }}" class="w-full py-4 bg-blue-600 text-white rounded-2xl font-bold text-sm shadow-lg shadow-blue-100 hover:bg-blue-700 transition-all active:scale-95 uppercase block text-center tracking-wide">
                                    <span>Pinjam Sekarang</span>
                                </a>
                            {{-- 🔥 FIX LIMIT CHECK: Disamakan menggunakan $liveActiveCount bawaan blade lo agar konsisten --}}
                            @elseif($liveActiveCount >= 5)
                            ...
                            @else
                                ...
                                <form action="{{ route('borrow.store', $book->id) }}" method="POST" class="borrow-form w-full m-0">
                                    @csrf
...
{{-- --- CORE ENGINE ANIMATION TIMING CODE --- --}}
<style>
