...
<body class="bg-slate-50 min-h-screen p-4 md:p-10">
    <div class="max-w-4xl mx-auto" x-data="{ openModal: false }">
        ...
        <a href="{{ route('dashboard') }}" class="inline-flex items-center text-sm font-bold text-slate-500 hover:text-blue-600 transition-all mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="md:col-span-1 bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col items-center">
                ...
                <h2 class="text-lg font-black text-slate-900 text-center">{{ auth()->user()->name }}</h2>
                <p class="text-sm text-slate-500 text-center mb-6">{{ auth()->user()->email }}</p>
                ...
                @forelse($lateBorrows as $loan)
                    <div class="bg-white p-5 rounded-3xl border-l-8 border-rose-500 shadow-sm flex justify-between items-center">
                        <div>
                            ...
                            <h3 class="font-bold text-slate-900 mt-1">{{ $loan->book->title }}</h3>
                            <p class="text-xs text-slate-400">Jatuh tempo: {{ \Carbon\Carbon::parse($loan->due_date)->format('d M Y') }}</p>
                        </div>
                        <div class="text-right">
                            <span class="block text-2xl font-black text-rose-600">
                                {{ \Carbon\Carbon::parse($loan->due_date)->diffInDays(now()) }}
                            </span>
            ...
            <div class="bg-white p-8 rounded-3xl w-full max-w-md shadow-2xl" @click.away="openModal = false">
                ...
                <form action="{{ route('profile.edit') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" value="{{ auth()->user()->name }}" class="w-full mt-2 p-3 bg-slate-50 rounded-xl font-bold border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none">
                    </div>
