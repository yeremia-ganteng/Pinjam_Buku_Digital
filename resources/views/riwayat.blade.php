...
{{-- MENYUNTIKKAN CDN SWEETALERT2 AGAR POPUP BERJALAN MULUS --}}
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <div class="flex items-center justify-between mb-8 text-left">
        ...
        <div class="wow-entrance" style="--delay: 150ms;">
            <a href="{{ route('welcome') }}" class="text-sm font-bold text-blue-600 hover:text-indigo-700 flex items-center transition-all group">
                        ...
                        <tr class="hover:bg-blue-50/30 transition-colors group">
                            <td class="py-6 px-8">
                                <div class="flex items-center">
                                    <div class="w-10 h-14 bg-slate-100 rounded-lg mr-4 overflow-hidden flex-shrink-0 shadow-sm border border-slate-100">
                                        <img src="{{ asset('storage/' . $borrow->book->image) }}" class="w-full h-full object-cover" onerror="this.src='https://images.unsplash.com/photo-1543002588-bfa74002ed7e?q=80&w=100'">
                                    </div>
                                    <span class="font-bold text-slate-700 group-hover:text-blue-600 transition-colors data-judul">{{ $borrow->book->title }}</span>
                                </div>
                            </td>
                            ...
                            <td class="py-6 px-8">
                                <span class="inline-flex items-center px-4 py-1.5 rounded-full text-xs font-black tracking-wide
                                    {{ Str::lower($borrow->status) === 'dipinjam' ? 'bg-blue-100 text-blue-700' : 'bg-emerald-100 text-emerald-700' }}">
                                    <span class="w-2 h-2 rounded-full mr-2 {{ Str::lower($borrow->status) === 'dipinjam' ? 'bg-blue-500' : 'bg-emerald-500' }}"></span>
                                    {{ $borrow->status }}
                                </span>
                            </td>
                            <td class="py-6 px-8">
                                @if(Str::lower($borrow->status) === 'dipinjam')
                                    {{-- FORM DIUPDATE DENGAN CLASS 'form-kembalikan' DAN CONVERT CONFIRM MANUAL KE INTERCEPTOR SWAL2 --}}
                                    <form action="{{ route('borrow.return', $borrow->id) }}" method="POST" class="form-kembalikan">
                                        @csrf
                        ...
                        <tr>
                            <td colspan="5" class="py-20 px-8 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <a href="{{ route('welcome') }}" class="mt-4 text-blue-600 font-bold hover:underline">Pinjam buku pertamamu sekarang →</a>
                                </div>
                            </td>
                        </tr>
                    @endforelse
