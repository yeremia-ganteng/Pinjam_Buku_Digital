...
<div class="dashboard-container">
    ...
    <div class="welcome-banner animate-item delay-1">
        <div class="welcome-text">
            <h1 class="text-3xl font-extrabold text-slate-900">Hi, {{ Auth::user()->name }} 👋</h1>
    ...
    <div class="menu-grid">
        ...
        {{-- CARD 1: KATALOG BUKU --}}
        <a href="{{ route('catalog') }}" class="nav-card animate-item delay-2">
        ...
        <div class="nav-card animate-item delay-3">
            ...
            <p class="text-sm text-slate-500 mb-4">
                Anda memiliki <strong>{{ str_pad($jumlahPinjam, 2, '0', STR_PAD_LEFT) }} Buku</strong> yang sedang dipinjam saat ini.
            </p>
            
            {{-- -- Bagian Logika Denda -- --}}
            @if(isset($totalDendaUser) && $totalDendaUser > 0)
                {{-- Jika ada denda, tampilkan kotak peringatan merah --}}
                <div class="mt-auto py-2 px-3 bg-rose-100 text-rose-700 text-[11px] font-bold rounded-lg w-full flex justify-between items-center border border-rose-200">
                    ...
                    <span>Rp {{ number_format($totalDendaUser, 0, ',', '.') }}</span>
                </div>
            @else
                {{-- WARNA DIKUNCI: Tetap Hijau Emerald segar & background card tetap putih bersih --}}
        ...
        {{-- CARD 3: RIWAYAT PINJAM --}}
        <a href="{{ route('riwayat') }}" class="nav-card animate-item delay-4">
...
{{-- CARD 4: PROFIL & BAR LIMIT (Otomatis Merah menyala jika >= 5 Buku) --}}
<a href="{{ route('profile.edit') }}" class="block nav-card animate-item delay-5 {{ $jumlahPinjam >= 5 ? 'border-limit-full' : '' }} hover:scale-[1.02] transition-transform duration-300">
    ...
    @endif
    <h3 class="font-bold text-xl mb-4 {{ $jumlahPinjam >= 5 ? 'text-limit-full' : 'text-slate-900' }}">Profil Member</h3>
    
    {{-- Progress Bar Kuota --}}
    <div class="w-full bg-slate-100 h-2.5 rounded-full overflow-hidden shadow-inner">
        <div class="h-full transition-all duration-500 rounded-full {{ $jumlahPinjam >= 5 ? 'bg-limit-progress' : 'bg-blue-600' }}" 
             style="width: {{ $persentase }}%"></div>
    </div>
    
    <span class="text-[10px] font-black mt-3 uppercase tracking-widest {{ $jumlahPinjam >= 5 ? 'text-rose-600' : 'text-slate-400' }}">
        Limit: {{ $jumlahPinjam }} / {{ $limitBuku }} Buku
    </span>
