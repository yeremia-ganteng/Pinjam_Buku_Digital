...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="stagger-animate w-full flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-slate-100 text-left" style="--d: 100ms;">
        <div class="flex-1 min-w-0">
            ...
            <div class="mt-4">
                <a href="{{ route('users.create') }}" class="inline-flex items-center px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-black tracking-wider uppercase rounded-xl border border-blue-700 shadow-md hover:shadow-xl hover:-translate-y-1 active:scale-95 transition-all duration-300">
                    ➕ Tambah Anggota Baru
                </a>
            </div>
        </div>
        ...
        <div class="flex-shrink-0 hidden md:block">
            <a href="{{ route('admin.dashboard') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-black tracking-wider uppercase rounded-xl border border-blue-700 shadow-md transition-all duration-300">
                ...
                <tbody class="divide-y divide-slate-100 bg-white">
                    @forelse($users as $index => $u)
                        <tr class="stagger-animate hover:bg-slate-50/40 transition-colors duration-150" style="--d: {{ 400 + ($index * 70) }}ms;">
                            <td class="py-4 px-6 sm:px-8 font-black text-slate-400 text-xs whitespace-nowrap text-left">#USR-{{ $u->id }}</td>
                            <td class="py-4 px-6 sm:px-8 font-bold text-slate-800 text-sm text-left">{{ $u->name }}</td>
                            <td class="py-4 px-6 sm:px-8 text-slate-600 text-sm text-left font-medium">{{ $u->email }}</td>
                            ...
                            <td class="py-4 px-6 sm:px-8 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center gap-4">
                                    ...
                                    <form action="{{ route('users.destroy', $u->id) }}" method="POST" class="inline form-delete">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" data-name="{{ $u->name }}"
