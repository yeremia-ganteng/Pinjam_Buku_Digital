        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden sm:flex sm:ms-10 items-center space-x-8">
                    @foreach(['Beranda' => 'welcome', 'Dashboard' => 'dashboard', 'Katalog' => 'catalog'] as $label => $route)
                        <a href="{{ route($route) }}" 
                           class="nav-item relative text-sm font-bold transition-colors duration-300 {{ request()->routeIs($route) ? 'nav-link-active text-gray-900' : 'text-gray-500 hover:text-gray-900' }}"
                           style="animation-delay: {{ 0.1 + ($loop->index * 0.15) }}s;">
                           {{ $label }}
                        </a>
                    @endforeach
                </div>
            ...
            <div class="hidden sm:flex items-center gap-3 nav-item" style="animation-delay: 0.5s;">
                ...
                <div x-data="{ open: false }" class="relative">
                    ...
                    {{-- DROPDOWN MENU --}}
                    {{-- Menggunakan fixed + z-index ekstrem untuk menjamin tampil di atas segalanya --}}
                    <div x-show="open" 
                        ...
                        <div class="max-h-40 overflow-y-auto custom-scrollbar pr-1">
                            ...
                            <a href="{{ route('dashboard') }}" class="block px-4 py-2.5 text-sm font-bold text-gray-700 hover:bg-gray-50 rounded-2xl transition-all">
                                Dashboard Utama
                            </a>

                            <a href="{{ route('profile.edit') }}" class="block px-4 py-2.5 text-sm font-bold text-gray-700 hover:bg-blue-50 hover:text-blue-600 rounded-2xl transition-all">
                                My Profile
                            </a>
                        </div>
                    </div>
...
<form id="logout-form-v2" method="POST" action="{{ route('logout') }}" class="hidden">@csrf</form>