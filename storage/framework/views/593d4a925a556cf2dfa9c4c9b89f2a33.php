...
<body class="bg-slate-50 min-h-screen p-8" x-data="{ showToast: <?php echo e(session('status') ? 'true' : 'false'); ?> }" x-init="setTimeout(() => showToast = false, 4000)">
    ...
    <div class="max-w-4xl mx-auto reveal-up" :class="{'active': true}">
        <a href="<?php echo e(route('dashboard')); ?>" class="text-sm font-bold text-slate-400 hover:text-blue-600 transition-colors">&larr; Kembali</a>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div class="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm text-center">
                ...
                <h2 class="mt-6 text-2xl font-black text-slate-900"><?php echo e(auth()->user()->name); ?></h2>
            </div>
                ...
                <form action="<?php echo e(route('profile.update')); ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="name" value="<?php echo e(auth()->user()->name); ?>" class="w-full mt-2 p-4 bg-slate-50 rounded-2xl border-none font-bold focus:ring-4 ring-blue-50">
                        </div>
                        <div>
                            <label class="text-[10px] font-black uppercase text-slate-400">Email</label>
                            <input type="email" name="email" value="<?php echo e(auth()->user()->email); ?>" class="w-full mt-2 p-4 bg-slate-50 rounded-2xl border-none font-bold focus:ring-4 ring-blue-50">
                        </div>
                    </div>
...
</html><?php /**PATH C:\yere\Herd\pinjam-buku\resources\views/profile/edit.blade.php ENDPATH**/ ?>