...
<div class="container mx-auto px-4 py-8 overflow-hidden">
    ...
    <div class="mb-8 opacity-0 animate-fade-in-left">
        <a href="<?php echo e(route('welcome')); ?>" class="inline-flex items-center text-blue-600 hover:text-blue-800 font-bold transition-all group">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4 opacity-0 animate-fade-in-down">
        ...
        <p class="text-slate-500 font-medium bg-slate-100 px-4 py-2 rounded-2xl text-sm self-start shadow-sm">
            Total: <?php echo e($books->total()); ?> Buku
        </p>
    </div>
    ...
    <div class="flex flex-col gap-6 mb-12 opacity-0 animate-fade-in-right">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
            <div class="hidden md:block">
                <span class="text-slate-500 font-bold bg-slate-100 px-5 py-3 rounded-2xl text-xs uppercase tracking-wider shadow-sm">
                    📚 Total: <?php echo e($books->total()); ?> Koleksi
                </span>
            </div>

            <div class="md:col-span-2 relative">
                <form action="<?php echo e(route('catalog')); ?>" method="GET" class="relative flex items-center">
        ...
        <div class="flex items-center gap-3 overflow-x-auto pb-2 no-scrollbar">
            ...
            <?php echo e(!request('category') ? 'bg-blue-600 text-white shadow-blue-200' : 'bg-white text-slate-500 border border-slate-100 hover:bg-slate-50'); ?>">
                ✨ Semua Koleksi
            </a>
            ...
            <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat => $icon): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <a href="<?php echo e(route('catalog', ['category' => $cat])); ?>"
                class="whitespace-nowrap px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-sm
                <?php echo e(strtolower(request('category')) == $cat ? 'bg-blue-600 text-white shadow-blue-200' : 'bg-white text-slate-500 border border-slate-100 hover:bg-slate-50'); ?>">
                    <?php echo e($icon); ?> <?php echo e(ucfirst($cat)); ?>
        ...
        <?php $__empty_1 = true; $__currentLoopData = $books; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $book): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <div class="scroll-reveal opacity-0 translate-y-12 bg-white shadow-sm border border-slate-100 rounded-[2.5rem] p-5 hover:shadow-2xl hover:shadow-blue-100 hover:-translate-y-2 transition-all duration-500 flex flex-col h-full min-h-[600px] transform-gpu">
                ...
                <div class="relative mb-6 overflow-hidden rounded-[2rem] shadow-sm aspect-[3/4]">
                    <img src="<?php echo e(Str::startsWith($book->image, 'http') ? $book->image : asset('storage/' . $book->image)); ?>" 
                    ...
                    <div class="absolute top-4 right-4">
                        <span class="px-3 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-xl shadow-sm
                            <?php if(Auth::check() && isset($book->is_borrowed_by_user) && $book->is_borrowed_by_user): ?>
                                bg-rose-500 text-white
                            <?php elseif($book->status === 'Tersedia' && $book->stock > 0): ?>
                ...
                <div class="flex flex-col flex-grow px-2">
                    <h3 class="text-lg font-black text-slate-800 line-clamp-2 leading-tight mb-2 uppercase tracking-tight"><?php echo e($book->title); ?></h3>
                    <p class="text-sm text-slate-400 font-bold mb-4 uppercase italic">By <?php echo e($book->author); ?></p>
                    ...
                    <div class="mb-4 flex items-center justify-between text-xs font-bold bg-slate-50 p-3 rounded-xl border border-slate-100">
                        ...
                        <?php if($book->stock > 0): ?>
                            <span class="text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-xl shadow-inner"><?php echo e($book->stock); ?> Buku</span>
                        <?php else: ?>
                    ...
                    <div class="mt-auto pt-4 border-t border-slate-50 flex flex-col gap-4">
                        <?php if($book->status === 'Tersedia' && $book->stock > 0): ?>
                            
                            
                            <div class="flex flex-col gap-3">
                                
                                <div class="relative">
                                    <select name="duration" form="form-pinjam-<?php echo e($book->id); ?>" class="w-full pl-4 pr-10 py-3 bg-slate-50 border-none rounded-xl text-xs font-bold text-slate-600 appearance-none focus:ring-2 focus:ring-blue-100 cursor-pointer uppercase">
                                ...
                                <div class="flex items-center justify-between gap-2 w-full">
                                    ...
                                    <?php if($book->stock <= 0): ?>
                                        <div class="w-full flex items-center justify-between gap-2">
                                            ...
                                            <a href="<?php echo e(route('user.books.show', $book->id)); ?>" class="p-4 bg-slate-50 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-2xl transition-colors" title="Detail Buku">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <?php elseif(Auth::check() && isset($book->is_borrowed_by_user) && $book->is_borrowed_by_user): ?>
                                        <div class="w-full flex items-center justify-between gap-2">
                                            <button disabled class="flex-grow bg-slate-100 text-slate-400 border border-slate-200 py-4 rounded-2xl font-black text-[10px] cursor-not-allowed uppercase tracking-wider flex items-center justify-center gap-1">
                                    ...
                                    <?php elseif(isset($activeBorrowCount) && $activeBorrowCount >= 5): ?>
                                        <div class="w-full flex items-center justify-between gap-2">
                                    ...
                                    <?php else: ?>
                                        ...
                                        <form id="form-pinjam-<?php echo e($book->id); ?>" action="<?php echo e(route('borrow.store', $book->id)); ?>" method="POST" class="borrow-form flex-grow m-0 w-full flex items-center justify-between gap-2">
                                            <?php echo csrf_field(); ?>
                            ...
                            <div class="flex flex-col gap-3">
                                ...
                                <div class="flex items-center justify-between gap-2">
                                    <button disabled class="flex-grow bg-slate-100 text-slate-400 py-4 rounded-2xl font-black text-[10px] cursor-not-allowed uppercase tracking-wider border border-slate-200">
                                        <?php echo e($book->stock <= 0 ? 'Stok Kosong' : 'Sedang Dipinjam'); ?>

                                    </button>
                                    <a href="<?php echo e(route('user.books.show', $book->id)); ?>" class="p-4 bg-slate-50 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-2xl transition-colors">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                        </svg>
                                    </a>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\yere\Herd\pinjam-buku\resources\views/catalog.blade.php ENDPATH**/ ?>